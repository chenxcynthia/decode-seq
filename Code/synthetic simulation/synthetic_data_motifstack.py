...
# ### Training + Testing synthetic data model
...
for i in range(num_top):
    # Calculate "weight"
    freq = (int(round(float(prediction[i, 0]), 1) * 10))
    #print(freq)
    # Add 'freq' number of kmers
    for j in range(freq):
        weighted_pred.append(prediction[i])
        
# prints top weighted predictions
#print('Positive kmer(s): ' + poskmer)
print('Weighted predictions: ')
print('\n'.join(np.asarray(weighted_pred)[:, 1]))


# In[54]:

prediction = np.asarray(weighted_pred)
...
plot_weights(weights, ', '.join(positive_kmers))
...
# # Determines clusters with weighted sequences
# cluster_pred = weighted_pred

# num_clusters = 5
# all_clusters = []
# for i in range(num_clusters):
#     cluster = []
#     top_kmer = cluster_pred[0]
#     for j in range(len(cluster_pred)):
#         if j >= len(cluster_pred): break
#         if kmer_similarity(top_kmer, cluster_pred[j]) > 1: #can change similarity calc
#             cluster.append(cluster_pred[j])
#             cluster_pred = np.delete(cluster_pred, j, 0)
#     all_clusters.append(cluster)
...
for i in range(num_clusters):
    ...
    plot_weights(clustered_weights[i],'Cluster ' + str(i+1))
    #fig, ax = plt.subplots(figsize=(10, 10))
    #plt.title('Cluster ' + str(i+1))
