...
class RepertoireModel:
    ...
    def objective(self, logits, labels):
        ...
        with tf.name_scope('objective'):
            cross_entropy = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=labels), axis=0)
...
class EncodingLayerModel(MultipleLabelModel):
    ...
    def __init__(self, num_motifs=100, encode_init=None, num_labels=2, model_name='EncodingLayerModel', save_path=''):
        ...
        if encode_init is None:
            ...
            self.encode_shape = (20, 20)
        elif type(encode_init) is int:
            self.encode_init = tf.contrib.layers.variance_scaling_initializer(factor=1.0, mode='FAN_IN', uniform=True)
            self.encode_shape = (20, encode_init)
        else:
    ...
    def pre_train(self, xs, cs, ys):
        batch_size, max_kmer, kmer_size, amino_acid = xs.shape
    ...
    def logits(self, features, counts, keep_prob):
        batch_size, max_kmer, kmer_size, amino_acid = features.get_shape()
        ...
        with tf.name_scope('layer0'):
            features_flat = tf.reshape(features, [batch_size*max_kmer*kmer_size, amino_acid])
            scores_flat = tf.matmul(features_flat, self.weights0)
            scores = tf.reshape(scores_flat, [batch_size*max_kmer, kmer_size*encode_size])

        with tf.name_scope('layer1'):
            scores_flat = tf.matmul(scores, self.weights1) + self.biases1
            scores = tf.reshape(scores_flat, [batch_size, max_kmer, self.num_motifs])
            counts_expand = tf.expand_dims(counts, axis=2)
            valid = tf.multiply(scores, tf.sign(counts_expand))
            logits1 = tf.reduce_max(valid, axis=1)
...
class GeneSwitchModel(EncodingLayerModel):
    ...
    def hidden_layers(self, features, counts, keep_prob):
        ...
        with tf.name_scope('encoding_layer'):
            scores = tf.reshape(features, [batch_size*max_kmer*kmer_size, amino_acid]) ## (B*M*K,A)
            scores = tf.matmul(scores, self.weights0) ## (A,E)
...
class GeneSwitchModelFast(GeneSwitchModel):
    ...
    def hidden_layers(self, features, counts, keep_prob):
        ...
        with tf.name_scope('encoding_layer'):
            scores = tf.reshape(scores, [-1, amino_acid]) ## (B*M*K,A)
            scores = tf.matmul(scores, self.weights0) ## (A,E)
            ## the shape is (B*M*K,E)
...
class DeepBCR(GeneSwitchModelFast):
    ...
    def __init__(self, num_motifs=30, encode_init=None, num_labels=2, neg_censor=True, model_name='DeepBCR', save_path=''):
        ...
        if num_labels == 1:
            ...
            raise ValueError('num_labels must > 1, == 1, or == -1')
    ...
    def set_run_mode(self, run_mode):
        options = ['Classification', 'Linear Regression', 'Cox-PH Regression']
    ...
    def logits(self, features, counts, keep_prob):
        ...
        with tf.name_scope('output_layer'):
            if self.run_mode == 'Classification':
                ...
                scores = tf.squeeze(tf.matmul(scores, self.weights3) + self.biases3) ## (N,1)
                ## the shape is (B,1)
            elif self.run_mode == 'Cox-PH Regression':
                scores = tf.squeeze(tf.matmul(scores, self.weights3) + self.biases3) ## (N,1)
                ## the shape is (B,1)
    ...
    def encode_labels(self, ys):
        if self.run_mode == 'Classification':
            ...
            ## No need to expand the integer labels in multiple label prediction anymore
            return np.array(ys, dtype=float)
        elif self.run_mode == 'Cox-PH Regression':
            ## Sorting on ys will be done in pre_train()
            return np.array(ys, dtype=float)
    ...
    def objective(self, logits, labels):
        ...
        if self.run_mode == 'Classification':
            ...
            with tf.name_scope('objective'):
                if self.neg_censor:
                    s1 = tf.boolean_mask(labels, labels >= 0)
                    s2 = tf.boolean_mask(logits, labels >= 0)
                else:
                    s1 = labels
                    s2 = logits
                ref = tf.norm(s1, ord=2)
                return tf.norm(s1-s2, ord=2)/ref - self.__correlation(s1, s2)
        elif self.run_mode == 'Cox-PH Regression':
            if self.neg_censor:
    ...
    def accuracy(self, logits, labels):
        ...
        elif self.run_mode == 'Linear Regression' or self.run_mode == 'Cox-PH Regression': ## TODO: use C-index
            with tf.name_scope('accuracy'):
                if self.neg_censor:
                    mask = labels >= 0
                    logits = tf.boolean_mask(logits, mask)
                    labels = tf.boolean_mask(tf.abs(labels), mask)
                else:
                    labels = tf.abs(labels)
...
def get_syn_data(num_samples=30, num_kmers=30, kmer_size=3, num_pos_kmers=3, 
                 aa_list='ACDEFGHIKLMNPQRSTVWY', positive=None):
    ''' Function for generating the synthetic data for deep_bcr models.
        ...
        Returns:
            xs: encoded repertoires, [num_samples, num_kmers, kmer_size, aa_size]
                where aa_size is the length of aa_list
    ...
    xs = np.zeros((num_samples, num_kmers, kmer_size, aa_size), dtype=np.float32) # Features
    cs = np.zeros((num_samples, num_kmers), dtype=np.float32) # Kmer count
    ...
    for i in range(num_samples):
        ...
        for j in range(N):
            ...
            for k in range(kmer_size):
                xs[i,j,k,np.random.randint(aa_size)] = 1.0 # Binary encoding
    ...
    for i in range(round(num_samples/2)):
        ...
        for k in range(len(kmer)):
            xs[i,j,k,:] = 0.0 # reset
            xs[i,j,k,aa_list.find(kmer[k])] = 1.0
...
if __name__ == '__main__':
    ...
    for m in [10, 50, 100]:
        for i in [1,2,4,8,16]:
            ...
            t1 = m1.train(xs.reshape(n,m,-1), cs, ys, save_log=SL, recover=RE)
            r1 = m1.test(XS.reshape(n,m,-1), CS, YS)
            m1.predict(XS.reshape(n,m,-1), CS)
            res.append((n,m,i,m1.model_name, r1[1][m1.bestfit_index], r1[2][m1.bestfit_index]))

            m2 = TwoLayerModel(save_path=model_path)
            t2 = m2.train(xs.reshape(n,m,-1), cs, ys, save_log=SL, recover=RE)
            r2 = m2.test(XS.reshape(n,m,-1), CS, YS)
            m2.predict(XS.reshape(n,m,-1), CS)
            res.append((n,m,i,m2.model_name,r2[1],r2[2][0]))

            m3 = MultipleLabelModel(save_path=model_path)
            t3 = m3.train(xs.reshape(n,m,-1), cs, ys, save_log=SL, recover=RE)
            r3 = m3.test(XS.reshape(n,m,-1), CS, YS)
            m3.predict(XS.reshape(n,m,-1), CS)
            ...
            m6 = DeepBCR(num_labels=-1, save_path=model_path)
