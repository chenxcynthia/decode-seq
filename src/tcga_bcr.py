...
def germline_kmers(k=6, trim_v=30,
        vfasta = '../data/IMGT_LIGM_DB/IGHV.fa', 
        jfasta = '../data/IMGT_LIGM_DB/IGHJ.fa'):

    ''' Return the set of k-mers from germline genes'''
    seqs = set()
    for s in read_fa_seq(vfasta):
        s = s[-trim_v:]
        if len(s) > 0:
            seqs.add(s)
    for s in read_fa_seq(jfasta):
        if len(s) > 0:
            seqs.add(s)
    mer = set()
    for s in seqs:
...
def load_data(meta, case, ratio=0.5, 
              datapath='../data/', 
              datafile='../data/TCGA/tcga_bcrh_tophat.txt.gz'):
    ...
    data_info = pd.DataFrame(list(zip(tumors, train_count, test_count)), columns=['tumors', 'train', 'test'])
...
def extract_bcr(trust_tab, rep_col='CDR3_aa', nrows=None):
    ''' Extract BCR repertorie for each patient 

        Args:
            trust_tab: data package for TRUST BCR
    ...
    '''
    tab = pd.read_csv(trust_tab, sep='\t', nrows=nrows)
...
## 61	Cell Line Derived Xenograft Tissue	XCL
    ...
    normal = tab[tab.Sample_Type.isin(['11'])] ## select normal samples
...
def rep2kmer(rep, k=6, ignore_germ=True):
    ''' Encode a repertoire using kmers 

        Args:
            rep:    a list of BCR CDR3s
            k:      the size of the kmer
            ignore_germ: whether or not ignore the germline kmers
...
def pid2label(pid, meta, use_subtype=True):
    ...
    if pd.isnull(cancer):
        return None
    if cancer in ['READ', 'COAD']: ## merge these two similar tumor types
        cancer = 'READ/COAD'
...
def get_next_batch(data, get_label, size=10, kmer=6, min_num=10):
...
def get_test_environment(data_file = '../data/TCGA/tcga_bcrh_tophat.txt.gz',
    ...
    data = extract_bcr(save_file, rep_col=col)
...
def test_survival_model(max_epoch=5):
    meta, data_train, data_test, tumors, model_path = get_test_environment(col=['CDR3_aa', 'Cgene'])

    ei = np.array([AA_CHEM[i] for i in AA_LIST], dtype='float')
    model = DeepBCR(num_labels=-1, encode_init=ei, save_path=model_path)

    labels = setup_survival(meta, tumors)
    encode = encode_aa_seq_binary_ext_dim
    count = encode_count_genes
    data_iter = sample_data_iter(data_train, 50, 6, 100, labels, format_trim_dims, encode, count)
    ...
    for i in range(max_epoch):
        train_res = model.train_batch(data_iter, max_iterations=(i+1)*100, it_step_size=20)
        print('> Performance of the survival model')
        test_res = model.test(xs, cs, ys)
